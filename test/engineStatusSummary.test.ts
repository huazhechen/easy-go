...
describe('engine status summary', () => {
  ...
  it('detects model source labels', () => {
    ...
    expect(getEngineModelSource('models/kata-small.bin.gz')).toBe('Bundled');
    expect(getEngineModelSource('/easy-go/models/katago-small.bin.gz')).toBe('Bundled');
    expect(getEngineModelSource('/easy-go/models/katago-small.bin.gz?t=1')).toBe('Bundled');
    expect(getEngineModelSource('https://example.com/model.bin.gz')).toBe('Remote');
    expect(getEngineModelSource('https://example.com/easy-go/models/katago-small.bin.gz')).toBe('Remote');
