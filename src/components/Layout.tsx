...
const MOBILE_HOME_DISMISSED_KEY = 'easy-go:mobile_home_dismissed:v1';
...
export const Layout: React.FC = () => {
  ...
  const [showSidebar, setShowSidebar] = useState(() => {
    return readLocalStorage('easy-go:sidebar_open:v1') !== 'false';
  });
  const [topBarOpen, setTopBarOpen] = useState(() => {
    return readLocalStorage('easy-go:top_bar_open:v1') !== 'false';
  });
  const [bottomBarOpen, setBottomBarOpen] = useState(() => {
    return readLocalStorage('easy-go:bottom_bar_open:v1') !== 'false';
  });
  ...
  const [leftPanelWidth, setLeftPanelWidth] = useState(() => {
    const raw = readLocalStorage('easy-go:left_panel_width:v1');
    const parsed = raw ? Number.parseInt(raw, 10) : NaN;
    return Number.isFinite(parsed) ? parsed : 300;
  });
  const [rightPanelWidth, setRightPanelWidth] = useState(() => {
    const raw = readLocalStorage('easy-go:right_panel_width:v1');
    const parsed = raw ? Number.parseInt(raw, 10) : NaN;
  ...
  useEffect(() => {
    ...
    // 3) File handler — files opened via the OS ("Open with Easy Go").
  ...
  useEffect(() => {
    writeLocalStorage('easy-go:sidebar_open:v1', String(showSidebar));
  }, [showSidebar]);

  useEffect(() => {
    writeLocalStorage('easy-go:top_bar_open:v1', String(topBarOpen));
  }, [topBarOpen]);

  useEffect(() => {
    writeLocalStorage('easy-go:bottom_bar_open:v1', String(bottomBarOpen));
  }, [bottomBarOpen]);

  useEffect(() => {
    writeLocalStorage('easy-go:left_panel_width:v1', String(leftPanelWidth));
  }, [leftPanelWidth]);

  useEffect(() => {
    writeLocalStorage('easy-go:right_panel_width:v1', String(rightPanelWidth));
  }, [rightPanelWidth]);
    ...
    >
      ...
      <LazyModalBoundary
        onError={(error) => {
          ...
          toast(
            stale
              ? 'Easy Go has been updated. Reload to open this.'
      ...
      <div className="flex flex-1 min-h-0 min-w-0 w-full overflow-hidden">
        ...
        >
          ...
          <h1 className="sr-only">Easy Go</h1>
