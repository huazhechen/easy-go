...
const LIBRARY_FOLDERS_EXPANDED_STORAGE_KEY = 'easy-go:library_folders_expanded:v1';
...
export const LibraryPanel: React.FC<LibraryPanelProps> = ({
  ...
  const [sortKey, setSortKey] = useState(() => {
    return readLocalStorage('easy-go:library_sort:v1') ?? 'recent';
  });
  const [listOpen, setListOpen] = useState(() => {
    return readLocalStorage('easy-go:library_list_open:v1') !== 'false';
  });
  ...
  useEffect(() => {
    writeLocalStorage('easy-go:library_sort:v1', String(sortKey));
  }, [sortKey]);

  useEffect(() => {
    writeLocalStorage('easy-go:library_list_open:v1', String(listOpen));
  }, [listOpen]);
  ...
  const handleBackupLibrary = () => {
    try {
      ...
      if (!downloadBlobFile(blob, `easygo-library-${new Date().toISOString().slice(0, 10)}.json`)) {
  ...
  const handleExportLibraryZip = async () => {
    try {
      ...
      downloadBlob(blob, `easygo-library-${new Date().toISOString().slice(0, 10)}.zip`);
  ...
  const handleBulkExport = async () => {
    ...
    try {
      ...
      downloadBlob(blob, `easygo-selection-${new Date().toISOString().slice(0, 10)}.zip`);
